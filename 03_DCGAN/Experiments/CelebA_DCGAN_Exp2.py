#**********************************************
# DCGAN in Pytorch Experiment 2 (depth --> 128,64, 32, 16, 3)
...
log_dir = home_path + 'logs/02_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp2'):
    os.mkdir('CelebA_DCGAN_results/Exp2')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp2'):
    os.mkdir('CelebA_DCGAN_results_10/Exp2')
...
for epoch in range(train_epoch):
    ...
    p = 'CelebA_DCGAN_results/Exp2/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    
    p_instance_ = 'CelebA_DCGAN_results_10/Exp2/CelebA_DCGAN__' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp2/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp2/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp2/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp2/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp2_generation_animation.gif', images, fps=5)
