#**********************************************
# DCGAN in Pytorch Experiment 10 $z \in R^{512}$ (depth --> 1024, 512, 256, 128, 3) 
...
class generator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.deconv1 = nn.ConvTranspose2d(512, d*8, 4, 1, 0)
...
log_dir = home_path + 'logs/10_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp10'):
    os.mkdir('CelebA_DCGAN_results/Exp10')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp10'):
    os.mkdir('CelebA_DCGAN_results_10/Exp10')
...
for epoch in range(train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        z_ = torch.randn((mini_batch, 512)).view(-1, 512, 1, 1)
        z_ = Variable(z_.cuda())
        G_result = G(z_)
    ...
    p = 'CelebA_DCGAN_results/Exp10/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    
    p_instance_ = 'CelebA_DCGAN_results_10/Exp10/CelebA_DCGAN__' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp10/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp10/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp10/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp10/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp10_generation_animation.gif', images, fps=5)
