...
class generator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.deconv1 = nn.ConvTranspose2d(100, d*16, 4, 1, 0)
...
data_dir = '/export/home/amanila/00_thesis/08_DCGAN/02_DCGAN_extend/data/resized_celebA'          # this path depends on your computer
...
if (temp.shape[0] != img_size) or (temp.shape[0] != img_size):
    sys.stderr.write('Error! image size is not 128 x 128! run \"celebA_data_preprocess.py\" !!!')
    sys.exit(1)

home_path = "/net/hci-storage02/userfolders/amanila/00_thesis/08_DCGAN/02_DCGAN_extend/"           # this path depends on your computer
log_dir = home_path + 'logs/13_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp13'):
    os.mkdir('CelebA_DCGAN_results/Exp13')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp13'):
    os.mkdir('CelebA_DCGAN_results_10/Exp13')
...
for epoch in range(train_epoch):
    ...
    p = 'CelebA_DCGAN_results/Exp13/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    p_instance = 'CelebA_DCGAN_results_10/Exp13/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result[0:10,:,:,:].data,'%s' % (p_instance), nrow=1,normalize=True)
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp13/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp13/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp13/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp13/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp13_generation_animation.gif', images, fps=5)
