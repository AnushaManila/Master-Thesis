...
# Added: #1. Prepared and trained the celebA dataset
         #2. Upsampling based on nearest neighbor interpolation
#**********************************************
...
class generator(nn.Module):
    ...
    def forward(self, input):
        ...
        #print('h_code0:',h_code) # 128X100X2X2
...
log_dir = home_path + 'logs/11_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp11'):
    os.mkdir('CelebA_DCGAN_results/Exp11')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
    if not os.path.isdir('CelebA_DCGAN_results_10/Exp11'):
    os.mkdir('CelebA_DCGAN_results_10/Exp11')
...
for epoch in range(train_epoch):
    ...
    for iter, (x_, _) in enumerate(train_loader):
        ...
        if (iter+1)%100 == 0:
            p_instance_ = 'CelebA_DCGAN_results_10/Exp11/CelebA_DCGAN__' + str(iter + 1) + '.png'
    ...
    p = 'CelebA_DCGAN_results/Exp11/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    p_instance = 'CelebA_DCGAN_results_10/Exp11/CelebA_DCGAN_' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp11/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp11/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp11/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp11/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp11_generation_animation.gif', images, fps=5)
