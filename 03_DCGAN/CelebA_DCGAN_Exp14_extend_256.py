...
# Added: #1. Prepared and trained the celebA dataset
         #2. Extend the architectures of G and D to generate 256X256 resolution images
#**********************************************
import os, time, sys
...
class discriminator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.conv7 = nn.Conv2d(d*32, 1, 4, 1, 0)
...
# training parameters
batch_size = 32
lr = 0.0007569 #0.00002 #0.000005 #0.000002
train_epoch = 20
...
data_dir = '/export/home/amanila/00_thesis/08_DCGAN/02_DCGAN_extend/data/resized_celebA_256'          # this path depends on your computer
dset = datasets.ImageFolder(data_dir, transform)
train_loader = torch.utils.data.DataLoader(dset, batch_size=32, shuffle=True)
...
log_dir = home_path + 'logs/14_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp14'):
    os.mkdir('CelebA_DCGAN_results/Exp14')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp14'):
    os.mkdir('CelebA_DCGAN_results_10/Exp14')
...
for epoch in range(train_epoch):
    ...
    for iter, (x_, _) in enumerate(train_loader):
        ...
        if (iter+1) % 500 == 0:
            p_iter = 'CelebA_DCGAN_results/Exp09/2CelebA_DCGAN_' + str(iter + 1) + '.png'
            vutils.save_image(G_result.data,'%s' % (p_iter), normalize=True)
    ...
    torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp14/generator_param.pkl")
    torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp14/discriminator_param.pkl")
    with open('CelebA_DCGAN_results/Exp14/train_hist.pkl', 'wb') as f:
        pickle.dump(train_hist, f)

    print('[%d/%d] - ptime: %.2f, loss_d: %.3f, loss_g: %.3f' % ((epoch + 1), train_epoch, per_epoch_ptime, torch.mean(torch.FloatTensor(D_losses)),
                                                              torch.mean(torch.FloatTensor(G_losses))))
    p = 'CelebA_DCGAN_results/Exp14/2CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    p_instance = 'CelebA_DCGAN_results_10/Exp14/2CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result[0:11,:,:,:].data,'%s' % (p_instance), nrow=3,normalize=True)
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp14/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp14_generation_animation2.gif', images, fps=5)
