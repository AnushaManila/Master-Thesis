#**********************************************
# DCGAN in Pytorch Experiment 8 $z \in R^{50}$ (depth --> 1024, 512, 256, 128, 3) 
...
class generator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.deconv1 = nn.ConvTranspose2d(50, d*8, 4, 1, 0)
...
log_dir = home_path + 'logs/08_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp8'):
    os.mkdir('CelebA_DCGAN_results/Exp8')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp8'):
    os.mkdir('CelebA_DCGAN_results_10/Exp8')
...
for epoch in range(train_epoch):
    ...
    p = 'CelebA_DCGAN_results/Exp8/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)
    
    p_instance_ = 'CelebA_DCGAN_results_10/Exp8/CelebA_DCGAN__' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp8/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp8/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp8/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp8/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp8_generation_animation.gif', images, fps=5)
