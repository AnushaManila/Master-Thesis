#**********************************************
# DCGAN in Pytorch Experiment 3 (depth --> 256,128,64, 32, 3)
...
log_dir = home_path + 'logs/03_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp3'):
    os.mkdir('CelebA_DCGAN_results/Exp3')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp3'):
    os.mkdir('CelebA_DCGAN_results_10/Exp3')
...
for epoch in range(train_epoch):
    ...
    p = 'CelebA_DCGAN_results/Exp3/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), normalize=True)

    p_instance = 'CelebA_DCGAN_results_10/Exp3/CelebA_DCGAN_' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp3/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp3/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp3/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp3/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp3_generation_animation.gif', images, fps=5)
