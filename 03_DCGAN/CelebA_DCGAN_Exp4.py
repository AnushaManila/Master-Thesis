#**********************************************
# DCGAN in Pytorch Experiment 4 (depth --> 512, 256, 128, 64, 32, 3)
...
log_dir = home_path + 'logs/04_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
...
if not os.path.isdir('CelebA_DCGAN_results/Exp4'):
    os.mkdir('CelebA_DCGAN_results/Exp4')
if not os.path.isdir('CelebA_DCGAN_results_10'):
    os.mkdir('CelebA_DCGAN_results_10')
if not os.path.isdir('CelebA_DCGAN_results_10/Exp4'):
    os.mkdir('CelebA_DCGAN_results_10/Exp4')
...
for epoch in range(train_epoch):
    ...
    p = 'CelebA_DCGAN_results/Exp4/CelebA_DCGAN_' + str(epoch + 1) + '.png'
    vutils.save_image(G_result.data,'%s' % (p), nrow=1, normalize=True)
   
    p_instance = 'CelebA_DCGAN_results_10/Exp4/CelebA_DCGAN_' + str(epoch + 1) + '.png'
        
    G_row = []
    for num in range(9):
        temp = G_result[num,:,:,:].data.cpu()
        temp_im = transforms.Compose([transforms.ToPILImage()])(temp)
        G_row.append(temp_im)
    G_row = np.concatenate(G_row, axis=1)
    imsave('%s' %(p_instance), G_row)
            
    p_instance_ = 'CelebA_DCGAN_results_10/Exp4/CelebA_DCGAN_' + str(epoch + 1) + '.png'
...
torch.save(G.state_dict(), "CelebA_DCGAN_results/Exp4/generator_param.pkl")
torch.save(D.state_dict(), "CelebA_DCGAN_results/Exp4/discriminator_param.pkl")
with open('CelebA_DCGAN_results/Exp4/train_hist.pkl', 'wb') as f:
...
for e in range(train_epoch):
    img_name = 'CelebA_DCGAN_results/Exp4/CelebA_DCGAN_' + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('CelebA_DCGAN_results/Exp4_generation_animation.gif', images, fps=5)
