...
# Added:  
        ...
        # 9. Function to get target eigen vectors
#**********************************************
import time
import os 
image_dir = os.getcwd() + '/Images/Bruegel/'
...
def get_targets_P(style_im,style_layers):
    # image to grids
    ...
    for l in range(5):
        ...
        s = s_to_choose[l]
        for z in range(9):
            gram = np.squeeze(style_im_grid_targets[z][l].data.cpu().numpy(), axis=(0,))
            lamda,P = eig_vec_sort(gram,s)
            grams_lamda.append(lamda)
            grams_P.append(P)
        style_gram_P.append(np.hstack(grams_P))
        style_gram_lamda.append(np.hstack(grams_lamda))
    return style_gram_lamda,style_gram_P
...
img_names = ['1.jpg', 'People.jpg']
imgs = [Image.open(img_dirs[i] + name) for i,name in enumerate(img_names)]
imgs_torch = [prep(img) for img in imgs]

style_layers = ['r11', 'r21', 'r31', 'r41', 'r51']
targets_lamda,targets_P = get_targets_P(imgs_torch[0],style_layers)
...
while n_iter[0] <= max_iter:
    ...
    def closure():
        ...
        for a, A in enumerate(out):
            ...
            s = s_to_choose[a]
            #s = targets[a].size()[1]
            lamda, P = targets_lamda[a],targets_P[a]
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/08_eig_grid_P/lr/1_people_eig_10percent_P.png', out_img)
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/08_eig_grid_P/hr/1_people_eig_10percent_P.png', out_img_hr)
