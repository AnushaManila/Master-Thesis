...
# Added:  
        ...
        # 7. Included AdaIN components instead of gram matrix style extraction
...
img_names = ['8.jpg', 'golden_gate.jpg']
d = 8
    ...
    def closure():
        ...
        for a, A in enumerate(out):
            if a == 5: # content layer doesn't need gram matrix computation
                ...
                print('loss_fns[a](A,targets[a]):',loss_fns[a](A,targets[a]))
                break
            if n_iter[0] == (max_iter-1):
                torch.save(targets[a], path+'lr/rainprincess_'+str(d)+'_targets[a]_relu'+str(a+1)+'_1')

            A_myu = A.mean(-1)
            stylized_myu = A_myu.mean(-1)
            A_sigma = A.std(-1)
            stylized_sigma = A_sigma.std(-1)

            targets_myu = (targets[a].mean(-1)).mean(-1)
            targets_sigma = (targets[a].std(-1)).std(-1)
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/09_AdaIn/lr/rainprincess_'+str(d)+'_golden_gate_AdaIn_StyleLoss_no_gram_e10.png', out_img)
    ...
    def closure():
        ...
        for a, A in enumerate(out):
            if a == 5:
                layer_losses.append(weights[a]*loss_fns[a](A,targets[a]))
                break
            if n_iter[0] == (max_iter_hr-1):
                torch.save(targets[a], path+'hr/rainprincess_'+str(d)+'_targets[a]_relu'+str(a+1)+'_1')
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/09_AdaIn/hr/rainprincess_'+str(d)+'_golden_gate_AdaIn_StyleLoss_no_gram_e10.png', out_img_hr)
