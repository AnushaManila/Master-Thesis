...
# Added:  
        ...
        # 8. Function to generate grids from style image and compute gram matricex and then follow procedure in Section 3.4
#**********************************************
import time
import os 
...
def get_style_targets(style_im,style_layers):
    # image to grids
    style_im_grid_targets = []
    row_counter = 0
    col_counter = 0
    start_i = 0
    i = 0
    j = 0
    while i < style_im.shape[1]:
        if row_counter == 0 :
            i = i + 170
        else:
            i = i + 171
        start_j = 0
        while j < style_im.shape[2]:
            if col_counter == 0:
                j = j + 170
            else:
                j = j + 171
            style_im_grid = style_im[:, start_i:i,start_j:j]
            if torch.cuda.is_available():
                style_im_grid_torch = Variable(style_im_grid.unsqueeze(0).cuda())
            else:
                style_im_grid_torch = Variable(style_im_grid.unsqueeze(0))
            style_im_grid_target = [GramMatrix()(A).detach() for A in vgg(style_im_grid_torch,style_layers)]
            style_im_grid_targets.append(style_im_grid_target)
            start_j = j
            col_counter = col_counter + 1
        j = 0
        start_i = i
        row_counter = row_counter + 1
    print('style_im_grid_target len:', len(style_im_grid_targets))
    # find the mean of gram matrices of grids
    l = 0
    style_gram = []
    for l in range(5):
        grams = []
        for z in range(len(style_im_grid_targets)):
            gram = style_im_grid_targets[z][l].data.cpu().numpy()
            grams.append(gram)
        gram_mat = np.mean(grams,axis=0)
        print('gram_mat shape:',gram_mat.shape)
        if torch.cuda.is_available():
            style_gram.append(Variable(torch.cuda.FloatTensor(gram_mat.tolist())))
        else:
            style_gram.append(Variable(torch.FloatTensor(gram_mat)))
    return style_gram
...
img_names = ['5.jpg', 'umbrella.jpg']
imgs = [Image.open(img_dirs[i] + name) for i,name in enumerate(img_names)]
imgs_torch = [prep(img) for img in imgs]

style_layers = ['r11', 'r21', 'r31', 'r41', 'r51']
style_targets = get_style_targets(imgs_torch[0],style_layers)
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/07_eig_grid/lr/5_umbrella_G2_eig_all.png', out_img)
...
imsave('/export/home/amanila/00_thesis/00_pytorch_neuralstyletransfer/PytorchNeuralStyleTransfer/outputs/07_eig_grid/hr/5_umbrella_G2_eig_all.png', out_img_hr)
