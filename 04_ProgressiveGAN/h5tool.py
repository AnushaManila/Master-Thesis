...
#Added: Saved image files separately.
...
class HDF5Exporter:
    ...
    def add_images(self, img):
        #print('inside add img:',img.shape)
...
def create_celeba_hq(h5_filename, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    glob_expected = 202599 #202600 
    """
    print('len(glob.glob(glob_pattern))',len(glob.glob(glob_pattern)))
    ...
    if md5.hexdigest() != '9cad8178d6cb0196b36f7b34bc5eb6d3':
    	#print('md5.hexdigest():', md5.hexdigest()) #f958616130d520c7d0cf2013b445c124
    ...
    def process_func(idx):
        ...
        # Apply delta image.
        img = img + delta
        #scipy.misc.imsave('img'+str(idx)+'.png',img)
        # Verify MD5.
        md5 = hashlib.md5()
        md5.update(img.tobytes())
    ...
    with ThreadPool(num_threads) as pool:
        ...
        for idx, img in pool.process_items_concurrently(fields['idx'], process_func=process_func, max_items_in_flight=num_tasks):
            ...
            scipy.misc.imsave('/export/home/amanila/00_thesis/09_pggan-pytorch/celebhq/img'+str(idx)+'.png',temp)
            h5.add_images(img[np.newaxis])
            #break
