...
for i in range(1, n_intp+1):
    z3 = torch.FloatTensor(1, config.nz).normal_(0.0, 1.0)
    z3_ = Variable(z3)
    fake_im = test_model.module(z3_)
    fname = os.path.join(name, '_extra{}.jpg'.format(i))
    utils.save_image_single(fake_im.data, fname, imsize=pow(2,config.max_resl))
    print('saved {}-th extra image ...'.format(i))
