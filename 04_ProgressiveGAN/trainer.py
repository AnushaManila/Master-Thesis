...
# Base: https://github.com/nashory/pggan-pytorch
# Added:
#       1. Prepared celebA HQ dataset as in Karras et al and trained the model to generate 512X512 resolution images
#**********************************************
...
class trainer:
    def __init__(self, config):
        ...
        if self.use_cuda:
            ...
            if config.n_gpu==1:
                self.G = torch.nn.DataParallel(self.G).cuda(0)
                self.D = torch.nn.DataParallel(self.D).cuda(0)
            else:
                gpus = []
